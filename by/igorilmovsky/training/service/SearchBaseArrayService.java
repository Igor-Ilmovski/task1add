...
public class SearchBaseArrayService {
    ...
    public int searchBaseArrayMin(BaseArray array) {
        int min = array.getBaseArrayItem(0);
        for (int i=0; i <array.getBaseArrayLength(); i++) {
            if (min > array.getBaseArrayItem(0)) {
                min = array.getBaseArrayItem(0);
            }
        }
    ...
    public int searchBaseArrayMax(BaseArray array) {
        int max = array.getBaseArrayItem(0);
        for (int i=0; i <array.getBaseArrayLength(); i++) {
            if (max < array.getBaseArrayItem(0)) {
                max = array.getBaseArrayItem(0);
            }
        }
    ...
    public int replaceBaseArrayItem(BaseArray array, int oldValue, int newValue) {
        int item;
        int replacedCount = 0;
        for (int i=0; i <array.getBaseArrayLength(); i++) {
            item = array.getBaseArrayItem(i);
            if (item == oldValue) {
               array.setBaseArrayItem(i, newValue);
               replacedCount++;
            }
        }
    ...
    public double getBaseArraySumm(BaseArray array) {
        ...
        for (int i = 0; i < array.getBaseArrayLength(); i++) {
            summ = summ + array.getBaseArrayItem(i);
        }
